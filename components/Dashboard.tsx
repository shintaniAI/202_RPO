...
import { TrendingUp, Users, MousePointerClick, Coins, Percent, Target, Info } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  const summary = useMemo(() => {
    ...
    return { totalCost, totalApps, totalClicks, avgCPA, ctr, cvr, avgCPC };
  }, [data]);
  ...
  const getBenchmarkStatus = (value: number, type: 'ctr' | 'cvr' | 'cpc' | 'cpa') => {
    switch(type) {
      case 'ctr': // 3-5%
        if (value < 3) return { color: 'text-red-500', text: '改善余地あり (<3%)' };
        if (value > 5) return { color: 'text-emerald-600', text: '良好 (>5%)' };
        return { color: 'text-slate-500', text: '平均的 (3-5%)' };
      case 'cvr': // 0.5-1%
        if (value < 0.5) return { color: 'text-red-500', text: '改善余地あり (<0.5%)' };
        if (value > 2) return { color: 'text-emerald-600', text: '非常に良好 (>2%)' };
        if (value > 1) return { color: 'text-blue-600', text: '良好 (>1%)' };
        return { color: 'text-slate-500', text: '平均的 (0.5-1%)' };
      case 'cpc': // 425
        if (value > 500) return { color: 'text-red-500', text: '高め (>¥500)' };
        if (value < 350) return { color: 'text-emerald-600', text: '良好 (<¥350)' };
        return { color: 'text-slate-500', text: '平均的 (¥425付近)' };
      case 'cpa': // 10000-15000
        if (value > 15000) return { color: 'text-red-500', text: '高め (>¥1.5万)' };
        if (value < 10000) return { color: 'text-emerald-600', text: '良好 (<¥1万)' };
        return { color: 'text-slate-500', text: '平均的 (¥1-1.5万)' };
      default:
        return { color: 'text-slate-500', text: '' };
    }
  };

  const ctrStatus = getBenchmarkStatus(summary.ctr, 'ctr');
  const cvrStatus = getBenchmarkStatus(summary.cvr, 'cvr');
  const cpcStatus = getBenchmarkStatus(summary.avgCPC, 'cpc');
  const cpaStatus = getBenchmarkStatus(summary.avgCPA, 'cpa');
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        <div className="group p-5 bg-slate-50 border-t-2 border-blue-900 transition-all hover:bg-white hover:shadow-lg hover:shadow-slate-200/50">
          ...
          <div className="mt-2 text-[10px] text-slate-400 flex items-center">
             <span className="font-medium">CPC: ¥{summary.avgCPC.toLocaleString()}</span>
             <span className={`ml-2 ${cpcStatus.color}`}>({cpcStatus.text})</span>
          </div>
        </div>
        ...
        <div className="group p-5 bg-slate-50 border-t-2 border-slate-300 transition-all hover:bg-white hover:shadow-lg hover:shadow-slate-200/50">
          ...
          <p className={`text-[10px] mt-1 ${cpaStatus.color}`}>{cpaStatus.text}</p>
        </div>
        <div className="group p-5 bg-slate-50 border-t-2 border-slate-300 transition-all hover:bg-white hover:shadow-lg hover:shadow-slate-200/50">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className={`text-[10px] mt-1 ${ctrStatus.color}`}>{ctrStatus.text}</p>
        </div>
        <div className="group p-5 bg-slate-50 border-t-2 border-slate-300 transition-all hover:bg-white hover:shadow-lg hover:shadow-slate-200/50">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className={`text-[10px] mt-1 ${cvrStatus.color}`}>{cvrStatus.text}</p>
        </div>
      </div>

      {/* Benchmark Info Box */}
      <div className="bg-slate-50 border border-slate-200 p-4 rounded text-xs text-slate-600">
        <div className="flex items-center mb-2 font-bold text-slate-700">
            <Info size={14} className="mr-2 text-blue-900"/>
            <span>Indeed平均データ基準</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
                <span className="block text-[10px] text-slate-400 uppercase">クリック率 (CTR)</span>
                <span className="font-medium">3〜5%</span>
                <span className="block text-[9px] text-slate-400">3%未満:改善余地 / 5%以上:良好</span>
            </div>
            <div>
                <span className="block text-[10px] text-slate-400 uppercase">応募率 (CVR)</span>
                <span className="font-medium">0.5〜1%</span>
                <span className="block text-[9px] text-slate-400">2%超:非常に良好</span>
            </div>
            <div>
                <span className="block text-[10px] text-slate-400 uppercase">クリック単価 (CPC)</span>
                <span className="font-medium">¥425</span>
            </div>
            <div>
                <span className="block text-[10px] text-slate-400 uppercase">応募単価 (CPA)</span>
